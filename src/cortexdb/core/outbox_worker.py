...
v2: Added _supervisor_loop() that monitors child tasks and restarts dead ones.
"""
...
class OutboxWorker:
    ...
    SUPERVISOR_INTERVAL = 10.0    # seconds between supervisor checks
    ...
    async def start(self):
        """Start the background polling, cleanup, and supervisor tasks."""
        if self._running:
            return
        self._running = True
    ...
    async def stop(self, timeout: float = 30.0):
        ...
        tasks = []
        for t in [self._poll_task, self._cleanup_task, self._recover_task, self._supervisor_task]:
            if t and not t.done():
                t.cancel()
                tasks.append(t)
    ...
    async def _supervisor_loop(self):
        """Monitor child tasks and restart any that have died unexpectedly."""
        while self._running:
            try:
            ...
            child_tasks = {
                "poll": (self._poll_task, self._poll_loop),
                "cleanup": (self._cleanup_task, self._cleanup_loop),
                "recover": (self._recover_task, self._recover_loop),
            }

            for name, (task, loop_fn) in child_tasks.items():
                if task is None or task.done():
                    if task and task.done() and not task.cancelled():
                        exc = task.exception()
                        if exc:
                            logger.error(f"OutboxWorker {name} task died: {exc}")
                    if self._running:
                        logger.warning(f"OutboxWorker restarting {name} task")
                        new_task = asyncio.create_task(loop_fn())
                        self._restarts_total += 1
                        if name == "poll":
                            self._poll_task = new_task
                        elif name == "cleanup":
                            self._cleanup_task = new_task
                        elif name == "recover":
                            self._recover_task = new_task

    @property
    def task_health(self) -> Dict[str, str]:
        """Return health status of each child task."""
        result = {}
        for name, task in [("poll", self._poll_task), ("cleanup", self._cleanup_task),
                           ("recover", self._recover_task), ("supervisor", self._supervisor_task)]:
            if task is None:
                ...
                result[name] = "running"
        result["restarts_total"] = self._restarts_total
        return result
    ...
    async def _mark_failed(self, conn: Any, entry_id: int,
                           ...
                           error_message: str):
        """Increment retry count with exponential backoff, or dead-letter."""
    ...
    async def get_metrics(self) -> Dict:
        """Return current outbox metrics (cached for METRICS_CACHE_TTL seconds)."""
        now = time.monotonic()
